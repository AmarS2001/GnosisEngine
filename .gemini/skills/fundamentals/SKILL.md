...
All topics live under the `docs/Fundamentals/` directory (create it if it
doesn't exist yet, and reuse it across runs — don't recreate it per topic).
Inside it, create a subfolder named after the canonical topic in kebab-case,
e.g. `docs/Fundamentals/kafka/` or `docs/Fundamentals/llm-inference/`. If
...
i.e. `docs/Fundamentals/<topic>/00-index.mdx`,
`docs/Fundamentals/<topic>/assets/...`, etc.

Also create `docs/Fundamentals/<topic>/_category_.json` for Docusaurus sidebar
grouping and collapsible navigation:
```json
{
...
comparisons & further resources. Aim for the smallest number of modules that
covers the topic properly (usually 6–12) — don't pad for padding's sake.
